...
const CheckOrders = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await fetch('${apiUrl}/Login/AdminPanel/Orders', {
          credentials: 'include',
        });
        if (response.ok) {
  ...
  const handleDeleteOrder = async () => {
    try {
      if (deleteOrderId) {
        const response = await fetch(`${apiUrl}/Login/AdminPanel/Orders/${deleteOrderId}`, {
          method: 'DELETE',
          credentials: 'include',
        });
  ...
  const handlePlaceOrder = async (order) => {
    try {
      ...
      const postResponse = await fetch(`${apiUrl}/Login/AdminPanel/Orders/PlacedOrder`, {
        method: 'POST',
        headers: {
      ...
      const emailResponse = await fetch('${apiUrl}:3001/send-Placed-confirmation', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
